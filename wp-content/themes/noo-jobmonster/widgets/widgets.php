...
class Noo_Jobs_Widget extends Noo_Widget {
	...
	public function widget( $args, $instance ) {
		...
		// --Check show type
		if ( $instance['show_type'] == 'slider' ) //  -- Check featured
		{
...
class Noo_Advanced_Job_Search_Widget extends WP_Widget {
	...
	public function widget( $args, $instance ) {
        ...
        <form id="<?php echo $prefix . '_form'; ?>" method="get" class="widget-advanced-search"
			...
			<?php endif; ?>
			<?php if ( $reset_button ) :?>
                <a href="<?php echo $url = is_tax() ? get_post_type_archive_link('noo_job') : '#'?>" class="btn btn-default reset-search"><i
		...
		<?php if ( ! $disable_live_search ) : ?>
            <script type="text/javascript">
                jQuery(document).ready(function ($) {
                    ...
                    if (container.length) {
                        $("#<?php echo $prefix . '_form'; ?>").on('change','select, input:not([type="checkbox"]):not([type="radio"])', function (event) {
                            event.preventDefault();
...
class Noo_Advanced_Company_Search_Widget extends WP_Widget{
    ...
    public function widget( $args, $instance ) {
        ...
        <form id="<?php echo $prefix . '_form'; ?>" method="get" class="widget-advanced-search"
            ...
            <?php endif; ?>
            <?php if ( $reset_button ) : ?>
                <a href="<?php echo $url = get_post_type_archive_link('noo_company')?>" class="btn btn-default reset-search"><i
                            class="fa fa-repeat"></i> <?php _e( 'Reset', 'noo' ); ?></a>
            <?php endif; ?>
        </form>
        <?php
...
class Noo_Social_Profile extends WP_Widget {
	...
	public function widget( $args, $instance ) {
		...
		$pinterest_profile = '<a class="instagram" href="' . $pinterest . '"><i class="fa fa-pinterest"></i></a>';
		$youtobe_profile   = '<a class="instagram" href="' . $youtobe . '"><i class="fa fa-youtube-play"></i></a>';
