...
?>
    <div class="member-manage">
        ...
        <?php if ($remain_featured_job > 0) : ?>
            <em><?php echo sprintf(_n('You can set %d more job to be featured. Featured jobs cannot be reverted.', 'You can set %d jobs to be featured. Featured jobs cannot be reverted.', $remain_featured_job, 'noo'), $remain_featured_job); ?></em>
        <?php endif; ?>
        <?php if ($remain_refresh_job > 0) : ?>
            <em><?php echo sprintf(__('You have %d times to refresh for all the job.', 'noo'), $remain_refresh_job); ?></em>
        <?php endif; ?>
        <form method="get">
                ...
                <table class="table noo-datatable" id="noo-table-job">
                    ...
                    <tr>
                        <th class="check-column">
                            <label class="noo-checkbox"><input type="checkbox"/><span
                                        class="noo-checkbox-label">&nbsp;</span></label>
                        </th>
                        ...
                        <th class="hidden-xs"><?php _e('Closing Date', 'noo') ?></th>
                        <th class="text-center"><?php _e('Apps', 'noo') ?></th>
                        <th class="text-center"><?php _e('Status', 'noo') ?></th>
                        <th data-priority="1" class="text-center"><?php _e('Action', 'noo') ?></th>
                    </tr>
                    </thead>
                    <tbody>
                    <?php if ($r->have_posts()) : ?>
                        <?php while ($r->have_posts()): $r->the_post();
                            ...
                            <tr>
                                <td class="check-column">
                                    <label class="noo-checkbox"><input type="checkbox" name="ids[]"
                                                                       value="<?php the_ID() ?>"><span
                                                class="noo-checkbox-label">&nbsp;</span></label>
                                </td>
                                <td>
                                    <?php if ('inactive' == $status || $status == 'pending' || $status == 'pending_payment' || $status == 'draft') :
                                ...
                                <td class="hidden-xs text-center">
                                    ...
                                    elseif ($can_set_featured_job) :
                                        ...
                                        <a href="<?php echo wp_nonce_url(add_query_arg(array(
                                            ...
                                            <span class="noo-job-feature not-featured" data-toggle="tooltip"
                                                  title="<?php _e('Set Featured', 'noo'); ?>"><i
                                                        class="fa fa-star-o"></i></span>
                                        </a>
                                    <?php else : ?>
                                        <span class="noo-job-feature not-featured"
                                              title="<?php _e('Set Featured', 'noo'); ?>"><i
                                                    class="fa fa-star-o"></i></span>
                                    <?php endif; ?>
                                </td>
                                ...
                                <td class="text-center" data-filter="<?php echo $status; ?>">
										<span class="jm-status jm-status-<?php echo esc_attr($status_class) ?>">
										<?php echo esc_html($status_text) ?>
										</span>
                                        <?php // Add button buy for customer
                                        //if('pending_payment' == $status_class):
                                            //$package_id = Noo_Job_Package::get_setting( 'package_page_id' );
                                            //if($package_id){
                                                //$package_url = get_permalink($package_id);?>
                                                <!-- <a title="<?php //echo esc_attr__('Buy a package', 'noo')?>" href="<?php //echo esc_url($package_url)?>"><i class="fa fa-arrow-circle-up"></i></a> -->
                                            <?php //}                                            
                                        //endif;?>
                                </td>
                                <td class="member-manage-actions text-center">
                                    ...
                                    $job_refresh = empty( $package ) || !is_array( $package ) || !isset( $package['job_refresh'] ) ? 0 : $package['job_refresh'];
                                    ?>
                                    <?php if($job_refresh > 0): ?>
                                        <span class="btn-refresh-job" data-id="<?php echo get_the_ID(); ?>"
                                              data-refresh="<?php echo $remain_refresh_job ?>"
                                              data-toggle="tooltip"
                                              title="<?php echo sprintf(esc_html__('Remain %s refresh time.', 'noo'), $remain_refresh_job); ?>">
                                            <i class="fa fa-refresh"></i>
                                        </span>
                                    <?php endif; ?>
                                    <span class="btn-clone-job" data-id="<?php echo get_the_ID(); ?>"
                                          data-toggle="tooltip"
                                          title="<?php esc_attr_e('Clone Job', 'noo') ?>">
                                    ...
                                    <?php if (Noo_Member::can_edit_job($post->ID, get_current_user_id())): ?>
                                        <a href="<?php echo Noo_Member::get_edit_job_url(get_the_ID()) ?>"
                                           ...
                                           title="<?php esc_attr_e('Edit Job', 'noo') ?>"><i
                                    ...
                                    <?php if($status == 'draft' || $status == 'expired'): ?>
                                        <a onclick="return confirm('<?php _e('Are you sure?', 'noo'); ?>')"
                                           ...
                                           data-toggle="tooltip" title="<?php esc_attr_e('Reposting', 'noo') ?>"><i
                                                    class="fa fa-repeat"></i></a>
                                    <?php endif; ?>
                                    <?php if ($status == 'expired') : ?>
                                        <a href="#" class="member-manage-action" data-toggle="tooltip"
                                           title="<?php esc_attr_e('Expired Job', 'noo') ?>"><i
                                    ...
                                    <a onclick="return confirm('<?php _e('Are you sure?', 'noo'); ?>')"
                                       ...
                                       data-toggle="tooltip" title="<?php esc_attr_e('Delete Job', 'noo') ?>"><i
                    ...
                    <?php else: ?>
                        <tr>
                            <td><a href="<?php echo Noo_Member::get_post_job_url(); ?>"
                                   class="btn btn-primary"><?php _e('Post Job', 'noo') ?></a></td>
                        </tr>
                    <?php endif; ?>
                    </tbody>
                </table>
