		...
		<form method="get">
			<div class="member-manage-toolbar top-toolbar clearfix">
				<div class="bulk-actions pull-left clearfix">
					...
					<button href="#"
					        class="btn btn-primary bulk-manage-application-action"><?php _e( 'Go', 'noo' ) ?></button>
				</div>
					...
					<div class="form-control-flat" style="width: 200px;">
						<select name="status" id="application_status">
							...
							<?php foreach ( $all_statuses as $key => $status ): ?>
								<option
									value="<?php echo esc_attr( $key ) ?>"><?php echo $status; ?></option>
							<?php endforeach; ?>
						</select>
						<i class="fa fa-caret-down"></i>
					</div>
			...
			<div class="noo-dashboard-table xxx">
				<table class="table noo-datatable" id="noo-table-app">
					...
					<tr>
                        ...
                        <th class="appl-col-avatar"><?php _e('Avatar','noo') ?></th>
						...
						while ( $r->have_posts() ): $r->the_post();
							...
							<tr>
								...
								<td>
									...
									if ( ! empty( $resume ) ) :
										...
										<a class="application-resume" data-application-id="<?php echo $post->ID; ?>"
										   ...
										   title="<?php echo esc_attr__( 'Resume', 'noo' ); ?>"><i
								...
								<td class="hidden-xs hidden-sm"><span><i
											class="fa fa-calendar"></i> <em><?php echo date_i18n( get_option( 'date_format' ), strtotime( $post->post_date ) ) ?></em></span>
								</td>
								...
								<td class="member-manage-actions text-center">
									...
									if ( $post->post_status == 'pending' ) :
										$approve_link = '<a href="#" class="member-manage-action approve-reject-action" data-hander="approve" data-application-id="' . $post->ID . '" data-toggle="tooltip" title="' . esc_attr__( 'Approve Application', 'noo' ) . '"><i class="fa fa-check-square-o"></i></a>';
										$reject_link  = '<a href="#" class="member-manage-action approve-reject-action" data-hander="reject"  data-application-id="' . $post->ID . '" data-toggle="tooltip" title="' . esc_attr__( 'Reject Application', 'noo' ) . '"><i class="fa fa-ban"></i></a>';
										...
										<a class="member-manage-action action-no-link"
										   title="<?php esc_attr_e( 'Approve Application', 'noo' ) ?>"><i
												class="fa fa-check-square-o"></i></a>
										<a class="member-manage-action action-no-link"
										   title="<?php esc_attr_e( 'Reject Application', 'noo' ) ?>"><i
												class="fa fa-ban"></i></a>
									<?php endif; ?>
									<?php
									...
									<a onclick="return confirm('<?php _e( 'Are you sure?', 'noo' ); ?>')"
									   ...
									   title="<?php esc_attr_e( 'Delete Application', 'noo' ) ?>"><i
					...
					<?php else: ?>
						<tr>
							<td colspan="8" class="text-center"><h3><?php _e( 'No Applications', 'noo' ) ?></h3>
							</td>
						</tr>
					<?php endif; ?>
					</tbody>
