		...
		<div id="<?php echo $package_style ; ?>" class="noo-pricing-table classic row package-pricing noo-col-<?php echo $columns; ?> <?php echo $package_style; ?>">
			<?php foreach ($packages as $package):?>
				<?php
                    ...
                    $job_refresh_text = sprintf( _n('%s refresh job', '%s refresh jobs', $refresh_limit, 'noo'), $refresh_limit );
				...
				<div class="<?php echo $product->get_slug(); ?> noo-pricing-column <?php echo 'col-sm-6 col-md-' . $columns_class; ?> <?php echo ( $product->is_featured() ? 'featured' : '' ); ?>">
				    <div class="pricing-content">
				        ...
				        <div class="pricing-info">
				            <ul class="noo-ul-icon fa-ul">
					            ...
					            <?php if( $refresh_limit > 0 ) : ?>
                                    <li class="noo-li-icon"><i class="fa fa-check-circle"></i> <?php echo $job_refresh_text; ?></li>
					            <?php else : ?>
                                    <li class="noo-li-icon"><i class="fa fa-times-circle-o not-good"></i> <?php echo __('No refresh job', 'noo'); ?></li>
					            <?php endif; ?>
				                <?php if( $is_unlimited || $job_limit > 0 ) : ?>
				        ...
				        if( Noo_Member::is_logged_in() ) {
			       			...
			       			<?php else : ?>
			       				<div class="pricing-footer" data-toggle="tooltip" title="<?php echo esc_html__('You cannot buy the package with a candidate account', 'noo'); ?>">
							        	<a style="pointer-events: none;" class="btn btn-lg btn-primary  btn-primary-disabled <?php echo ( ($purchased_free_package && $product->get_price() <= 0 ) ? 'disabled' : ''); ?> <?php echo ($product->get_price() == 0 && is_user_logged_in() ) ? ' auto_create_order_free' : ''; ?>" data-id="<?php echo get_current_user_id(); ?>"<?php echo ($product->get_price() == 0 && is_user_logged_in() ) ? ' data-security="' . wp_create_nonce( 'noo-free-package' ) . '" data-url-package="' . $redirect_package_free . '"' : ' href="'.$checkout_url_candidate.'"'; ?> data-package="<?php echo $product->get_id() ?>"><?php echo wp_kses_post($product->add_to_cart_text())?></a>
							    </div>
			       			<?php endif; ?>
			       		<?php }else{?>
				       		<?php $link = Noo_Member::get_login_url();?>
				       		<div class="pricing-footer">
						        	<a class="btn btn-lg btn-primary <?php echo ( ($purchased_free_package && $product->get_price() <= 0 ) ? 'disabled' : ''); ?> <?php echo ($product->get_price() == 0 && is_user_logged_in() ) ? ' auto_create_order_free' : ''; ?>" data-id="<?php echo get_current_user_id(); ?>"<?php echo ($product->get_price() == 0 && is_user_logged_in() ) ? ' data-security="' . wp_create_nonce( 'noo-free-package' ) . '" data-url-package="' . $redirect_package_free . '"' : ' href="' . esc_url($link) .'"'; ?> data-package="<?php echo $product->get_id() ?>"><?php echo wp_kses_post($product->add_to_cart_text())?></a>
						        </div>
				      <?php }?>
				        <?php if( !empty( $package->post_content ) ) : ?>
