...
<?php elseif ($style == 'style2' || $style==''): ?>
	...
	$feed        = $main_url . 'feed/company_feed';
                            ...
                            <div class="company-item-meta">
                                ...
                                <p>
                                    <i class="fa fa-briefcase"></i><span
                                            class="job-count"><?php echo $count > 0 ? sprintf(_n('%s Job', '%s Jobs', $count, 'noo'), $count) : __('No Job', 'noo'); ?></span>
                                </p>
                            </div>
                        </div>
                    </div>
...
<?php elseif($style == 'style1') : ?>
	...
	foreach ($letter_range as $letter)
	{
		...
		$company_query      = "SELECT p.ID, p.post_title, p2.total_company 
                                   ...
                                   WHERE p.post_type = 'noo_company' AND p.post_status = 'publish' AND p.post_title <> '' AND p.post_title LIKE '{$letter}%'
	...
	$other_companies_query = "SELECT p.ID, p.post_title
                                   FROM {$wpdb->prefix}posts as p 
                                   WHERE p.post_type = 'noo_company' AND p.post_status = 'publish' AND p.post_title <> '' {$not_like_str}
                                   ORDER BY p.post_title ASC";
