    ...
    function noo_shortlist() {
	    ...
	    if ( !empty( $_POST['resume_id'] ) && !empty( $_POST['user_id'] ) && !empty( $_POST['type'] ) ) {
		    ...
		    if ( in_array( $user_id, $list_shortlist ) ) {
			    ...
			    $response['label'] = ($type == 'text' ? esc_html__( 'ShortList', 'noo' ) : '<i class="fa fa-heart-o" aria-hidden="true"></i>' );
		    } else {
    ...
    function noo_resume_contact() {
	    ...
	    if ( !empty( $_POST['candidate_id'] )) {
		    ...
		    if ( $candidate ) {
			    ...
			    $result = noo_mail( $email, $subject, $message, array(), 'noo_send_contact_resume' );
			    if ( $result ) {
				    $response['status'] = 'success';
