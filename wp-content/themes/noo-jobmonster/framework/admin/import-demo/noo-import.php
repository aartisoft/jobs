...
class Noo_Import_Demo extends WP_Importer{
	...
	public function process_data() {
		...
		// --- load file demo
			...
			$slider_file = NOO_FRAMEWORK_ADMIN . "/import-demo/data/{$_POST['name']}/home-slider.zip";

			$this->import( $file_demo );
			$this->_noo_process_import_file( $widget );
			$this->process_option( $option );
			$this->import_RevSlider( $slider_file );
	...
	function import_RevSlider($slider_file)
	{
	...
	function _noo_import_data( $data ) {
		...
		if ( empty( $data ) || ! is_object( $data ) ) {
			wp_die(
				esc_html__( 'Imported data could not be read. Please try a different file..', 'noo' ),
				'',
		...
		foreach ( $data as $sidebar_id => $widgets ) {
			...
			if ( isset( $wp_registered_sidebars[$sidebar_id] ) ) {
				...
				$sidebar_message = esc_html__( 'Sidebar does not exist in theme (using Inactive)', 'noo' );
			}
			...
			foreach ( $widgets as $widget_instance_id => $widget ) {
				...
				if ( ! $fail && ! isset( $available_widgets[$id_base] ) ) {
					...
					$widget_message = esc_html__( 'Site does not support widget', 'noo' ); // explain why widget not imported
				}
				...
				if ( ! $fail && isset( $widget_instances[$id_base] ) ) {
					...
					foreach ( $single_widget_instances as $check_id => $check_widget ) {
						...
						if ( in_array( "$id_base-$check_id", $sidebar_widgets ) && (array) $widget == $check_widget ) {
							...
							$widget_message = esc_html__( 'Widget already exists', 'noo' ); // explain why widget not imported
				...
				if ( ! $fail ) {
					...
					if ( $sidebar_available ) {
						...
						$widget_message = esc_html__( 'Imported', 'noo' );
					} else {
						$widget_message_type = 'warning';
						$widget_message = esc_html__( 'Imported to Inactive', 'noo' );
					}
				...
				$results[$sidebar_id]['widgets'][$widget_instance_id]['title'] = ! empty( $widget['title'] ) ? $widget['title'] : esc_html__( 'No Title', 'noo' ); // show "No Title" if widget instance is untitled
	...
	private function get_whitelist_options() {
		return apply_filters( 'options_import_whitelist', array(
			...
			'noo_candidate_custom_field',
			'noo_company_custom_field',
