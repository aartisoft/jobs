...
function show_more_company() {
	...
	if( !isset( $_POST['filter'] ) || $_POST['filter'] == '' || !isset( $_POST['start'] ) || $_POST['start'] == '' || !is_numeric($_POST['start']) ) {
		$result = array(
			'success' => false,
			'html'    => '',
		);
	} else
	{
		...
		$filter = $_POST['filter'];
		$query = "SELECT p.ID, p.post_title FROM {$wpdb->prefix}posts as p WHERE p.post_type = 'noo_company' AND p.post_status = 'publish' AND p.post_title <> '' AND p.post_title LIKE '{$filter}%' ORDER BY p.post_title ASC LIMIT $start, $limit_show";
